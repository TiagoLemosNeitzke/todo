<x-app-layout>
    <div class="bg-gray-700 w-full h-screen flex flex-row justify-center">
        <div class="flex flex-col justify-center items-center text-center">
            ...
            <form action="{{route('addMember')}}" method="post" class="bg-white rounded-b-md w-60">
                @csrf
                <div class="flex flex-col justify-start items-center p-4 pb-6">
                    <label>
                        <input class="rounded-md bg-slate-300 text-xs border-none" type="email" id="email" name="email" placeholder="User email to add to team" required>
                    </label>
                    @error('email')
                    <div class="mt-2">
